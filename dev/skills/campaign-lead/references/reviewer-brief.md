...
1. **The architecture contract.** Point it at the campaign's own contract sections (the
   `C<n>` sections of its architecture report) so it checks the merged code against stated
   invariants, not against its own guess at what "correct" means.
...
3. **Instructions to probe every validator with deliberately invalid input, not just read
   ...
   a bad `--message` instead. Give a drift test a deliberately broken fixture. Validator
   defects are found this way, never by reading valid history (KT-H §Review process changes,
   OBS-H).
...
130-line cap never crosses the cap, and the probe passes without exercising anything (OBS-D
F2). Ask for "enough to exceed the cap" and have the reviewer report the size used.
