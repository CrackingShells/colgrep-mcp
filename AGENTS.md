...
| `plugin.json`, `mcp.json`, `.claude-plugin/`, `.codex-plugin/`, `.agents/` | product plugin manifests for the three ecosystems; the Claude Code and Codex MCP config is `.claude-plugin/mcp.json`; `version` fields are written by `cz bump` only | launcher/packaging changes |
...
| `.github/workflows/ci.yml` | ruff check, ruff format check, pytest on three OSes, `cz check` on pull requests | when gates change |
